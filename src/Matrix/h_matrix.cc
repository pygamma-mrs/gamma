...
// are installed. Otherwise we will just use the standard mxexp() which uses diagonalization.
  ...
  if(rows_ != cols_)
  { HMxerror(14,1);			//    Bad rectangular array use
    HMxfatal(28);			//    Unable to diagonalize
  }
//std::cerr << "At Point 01 h_matrix.cc\n";
  L1=0;
  for(k1=0;k1<size;++k1)
  { temp = norm(data[k1]);
    if(temp > L1)
...
//        V = b[8]*A8+b[6]*A6+b[4]*A4+b[2]*A2+b[0]*ident;
        ...
        { maxnorm = 5.371920351148152e0;
          n_squ = std::max(0, int(std::ceil(log2(L1/maxnorm))));
